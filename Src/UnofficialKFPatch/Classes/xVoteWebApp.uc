...
final function AddConfigEditbox( WebAdminQuery q, string InfoStr, coerce string CurVal, int MaxLen, string ResponseVar, string Tooltip, optional bool bSkipTrail, optional float Increments, optional bool bInt, optional float MinValue=0.f, optional float MaxValue=1000.f )
{
    ...
    if( InfoStr == "" )
        S = "<abbr title=\""$Tooltip$"\"><TD>";
    else S = "<TR><TD><abbr title=\""$Tooltip$"\">"$InfoStr$":</abbr></TD><TD>";
    S $= "<input type=\"text\" id=\""$ResponseVar$"\" size=\""$Clamp(MaxLen,15,100)$"\" name=\""$ResponseVar$"\" value=\""$CurVal$"\""$(Increments != 0.f ? "" : " style=\"border: 1px solid #181818; background-color: #0a0a0a;\"")$" class=\""$(Increments != 0.f ? "spin-button" : "textbox")$"\">";
    if( Increments != 0.f )
        S $= "<script type=\"text/javascript\"> $(document).ready(function(){ var elm = $('#"$ResponseVar$"'); elm.numeric(); elm.SpinButton({min: "$MinValue$", max: "$MaxValue$", step: "$Increments$", page: "$Increments$"*10, asint: "$(bInt ? "true" : "false")$"}); }); </script>";
    S $= "</TD>";
...
function handleMapVotes(WebAdminQuery q)
{
	...
	for( i=0; i<class'xVotingHandler'.Default.GameModes.Length; ++i )
	{
		if( EditSettingLine==i )
		{
			...
			AddInLineEditbox(q,class'xVotingHandler'.Default.GameModes[i].GameName,MaxInt,"GN","Game type long display name");
			AddInLineEditbox(q,class'xVotingHandler'.Default.GameModes[i].GameShortName,MaxInt,"GS","Game type short display name");
			AddInLineEditbox(q,class'xVotingHandler'.Default.GameModes[i].GameClass,MaxInt,"GC","Game type class name to run");
