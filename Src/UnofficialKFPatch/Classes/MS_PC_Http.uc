Class MS_PC_Http extends KFPlayerController;

var byte ConnectionCounter;
var bool bConnectionFailed;
var transient MS_Game_Http Game;
    ...
    if( ConnectionCounter<3 && ++ConnectionCounter==3 )
    {
        if( Game.PendingURL!="" )
        {
            if( Game.bServerHidden )
                MS_HUD_Http(myHUD).ShowProgressMsg("Connecting to Private Match");
            else MS_HUD_Http(myHUD).ShowProgressMsg("Connecting to "$Game.PendingURL);
...
reliable client function bool ShowConnectionProgressPopup( EProgressMessageType ProgressType, string ProgressTitle, string ProgressDescription, bool SuppressPasswordRetry = false)
{
    ...
    case PMT_PeerConnectionFailure:
        ...
        MS_HUD_Http(myHUD).ShowProgressMsg("Connection Error: "$ProgressTitle$"|"$ProgressDescription$"|Disconnecting...",true);
        SetTimer(4,false,'HandleNetworkError');
        return true;
    case PMT_DownloadProgress:
    case PMT_AdminMessage:
        MS_HUD_Http(myHUD).ShowProgressMsg(ProgressTitle$"|"$ProgressDescription);
        return true;
    }
    return false;
}
...
defaultproperties
{
    InputClass=class'MS_Input_Http'
}